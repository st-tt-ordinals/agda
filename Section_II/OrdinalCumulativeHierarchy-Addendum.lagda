...
\begin{code}
  ...
  module small-quotient-as-ordinal
   ...
   []⁻-is-surjection =
    ∘-is-surjection
     (surjection-induction-converse [_] (λ P → /-induction ~EqRel))
...
equals the recursive supremum given by Ψᵒʳᵈ (𝕍-set f).

This is indeed the case and because Φᵒʳᵈ is left-cancellable, it suffices
to show that
  Φ (A/~ᵒʳᵈ) ＝ 𝕍-set f.
        ...
        ⦅1⦆ x m = ∥∥-rec ∈-is-prop-valued ⦅1⦆' fact
         where
          ...
          lemma b n e = transport (_∈ f a) (e' ⁻¹) n
           where
            e' = x                   ＝⟨ e                            ⟩
                 Φ (A/~⁻ᵒʳᵈ ↓ [ b ]⁻) ＝⟨ IH [ b ]⁻ (∈-to-≺⁻ n) b refl ⟩
                 f b                ∎

          fact : ∃ b' ꞉ ⟨ ↓a ⟩ , Φ (↓a ↓ b') ＝ x
          fact = from-∈-of-𝕍-set (transport (x ∈_) (Φ-behaviour ↓a) m)

          ⦅1⦆' : (Σ b' ꞉ ⟨ A/~⁻ᵒʳᵈ ↓ [ a ]⁻ ⟩ , Φ (↓a ↓ b') ＝ x)
              → x ∈ f a
        ...
        ⦅2⦆ x m = ∥∥-rec ∈-is-prop-valued (λ (b , n , e) → ⦅2⦆' b n e) fact
         where
          ...
          ⦅2⦆' : (b : A)
               ...
               → f b ＝ x
               → x ∈ Φ ↓a
          ⦅2⦆' b n e = transport (_∈ Φ ↓a) (lemma b n e) mem
           where
            mem' : Φ (A/~⁻ᵒʳᵈ ↓ [ b ]⁻) ∈ 𝕍-set (λ b' → Φ (↓a ↓ b'))
            mem' = to-∈-of-𝕍-set ∣ ([ b ]⁻ , ∈-to-≺⁻ n) , e' ∣
             where
              e' : Φ (↓a ↓ ([ b ]⁻ , ∈-to-≺⁻ n))
                 ＝ Φ (A/~⁻ᵒʳᵈ ↓ [ b ]⁻)
              e' = ap Φ (iterated-↓ A/~⁻ᵒʳᵈ [ a ]⁻ [ b ]⁻ (∈-to-≺⁻ n))
            mem : Φ (A/~⁻ᵒʳᵈ ↓ [ b ]⁻) ∈ Φ ↓a
            mem = transport⁻¹ (Φ (A/~⁻ᵒʳᵈ ↓ [ b ]⁻) ∈_)
                              (Φ-behaviour ↓a)
                              mem'

\end{code}

Using that Φᵒʳᵈ is left-cancellable and a retraction of Ψᵒʳᵈ, we
now prove that the recursive supremum given by Ψᵒʳᵈ (𝕍-set f) is equal to
...
\begin{code}
   ...
   Ψᵒʳᵈ-is-quotient-of-carrier : Ψᵒʳᵈ (𝕍-set f , σ) ＝ A/~⁻ᵒʳᵈ
   Ψᵒʳᵈ-is-quotient-of-carrier =
    Φ-is-left-cancellable (Ψᵒʳᵈ (𝕍-set f , σ)) A/~⁻ᵒʳᵈ e
     where
      e = Φ (Ψᵒʳᵈ (𝕍-set f , σ))          ＝⟨ ap pr₁ ⦅1⦆        ⟩
          𝕍-set f                         ＝⟨ 𝕍-set-ext _ _ ⦅2⦆ ⟩
          𝕍-set (λ a' → Φ (A/~⁻ᵒʳᵈ ↓ a')) ＝⟨ ⦅3⦆               ⟩
          Φ A/~⁻ᵒʳᵈ                       ∎
       where
        ⦅1⦆ : Φᵒʳᵈ (Ψᵒʳᵈ (𝕍-set f , σ)) ＝ 𝕍-set f , σ
        ⦅1⦆ = Ψᵒʳᵈ-is-section-of-Φᵒʳᵈ (𝕍-set f , σ)
        ⦅2⦆ : f ≈ (λ a' → Φ (A/~⁻ᵒʳᵈ ↓ a'))
        ⦅2⦆ = ⦅2⦆ˡ , ⦅2⦆ʳ
         where
          ⦅2⦆ˡ : f ≲ (λ a' → Φ (A/~⁻ᵒʳᵈ ↓ a'))
...
(large) ordinals we distill a proof that Ψᵒʳᵈ x is isomorphic as an
...
\begin{code}
 ...
 module _
  ...
  Ψᵒʳᵈ-is-isomorphic-to-total-space :
     (x : 𝕍) (σ : is-set-theoretic-ordinal x)
   → Ψᵒʳᵈ (x , σ) ≃ₒ total-spaceᵒʳᵈ x σ
  Ψᵒʳᵈ-is-isomorphic-to-total-space = 𝕍-prop-simple-induction _ prop-valued γ
   where
    prop-valued : (x : 𝕍)
                → is-prop ((σ : is-set-theoretic-ordinal x) → Ψᵒʳᵈ (x , σ)
                                                            ≃ₒ total-spaceᵒʳᵈ x σ)
    prop-valued x = Π-is-prop fe (λ σ → ≃ₒ-is-prop-valued _ _)
    γ : {A : 𝓤 ̇ } (f : A → 𝕍) (σ : is-set-theoretic-ordinal (𝕍-set f))
      → Ψᵒʳᵈ (𝕍-set f , σ) ≃ₒ total-spaceᵒʳᵈ (𝕍-set f) σ
    γ {A} f σ = ≃ₒ-trans (Ψᵒʳᵈ (𝕍-set f , σ))
                         A/~⁻ᵒʳᵈ
     ...
     where
      ...
      open quotient-as-ordinal σ
      ⦅1⦆ : Ψᵒʳᵈ (𝕍-set f , σ) ≃ₒ A/~⁻ᵒʳᵈ
      ⦅1⦆ = idtoeqₒ _ _ Ψᵒʳᵈ-is-quotient-of-carrier
